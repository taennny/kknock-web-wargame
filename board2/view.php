      ...
      <div class="card">
        ...
        <?php if ($post['file_name']): ?>
          <p class="file-download"><strong>첨부파일:</strong>
            <a href="../upload/board2/<?= urlencode($post['file_name']) ?>" download>
        ...
        <?php endif; ?>
         ...
         <?php if ($userId == $post['user_id']): ?>
          <div class="post-actions">
            ...
            <a href="delete.php?id=<?= $post['id'] ?>" onclick="return confirm('정말 삭제하시겠습니까?')">delete</a>
          </div>
        <?php endif; ?>
        ...
        <!-- 댓글 목록 -->
        <?php
        $stmt = $pdo->prepare("SELECT c.*, u.name FROM comments_board2 c JOIN users u ON c.user_id = u.id WHERE c.post_id = ? ORDER BY c.created_at ASC");
